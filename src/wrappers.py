...
class ObservationWrapper(gym.ObservationWrapper):
    ...
    def __init__(self, env, shape=(84,84), color_image = False):
        super().__init__(env)
        ...
        if color_image:
            ...
            num_channels = 1
        new_shape = (num_channels, shape[0], shape[1])
        self.observation_space = gym.spaces.Box(
            0, 1, shape=new_shape, dtype=np.uint8
        )
    ...
    def observation(self, observation):
        if self.color_image:
            ...
            observation = cv2.cvtColor(observation["screen"], cv2.COLOR_RGB2GRAY)
